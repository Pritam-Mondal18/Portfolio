...
const Navbar = () => {
    ...
    const observeSections = () => {
      ...
      sections.forEach((id) => {
        ...
        if (section && !sectionsRef.current.includes(id)) {
          ...
          console.warn(
            `Section with id "${id}" not found. Ensure it exists in your components.`
          );
        }
      });
    };
  ...
  const handleDownload = () => {
    setIsLoading(true);
    const cvFile = "/Pritam_CV.pdf";
    const link = document.createElement("a");
    link.href = cvFile;
    link.download = "Pritam_CV.pdf";
        ...
        <div className="flex items-center justify-between h-20">
          <div className="flex items-center">
            <Link
              ...
              onClick={() => handleNavClick({ name: "Home", href: "/#hero" })}
              aria-label="Home"
            >
              <img
                src={logo}
                ...
                loading="lazy"
              />
              <span className="text-white text-2xl font-semibold">Pritam</span>
            </Link>
          </div>
